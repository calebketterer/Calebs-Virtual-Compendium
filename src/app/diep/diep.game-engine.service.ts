...
export class DiepGameEngineService {
    ...
    public resetState(startGameImmediately: boolean) {
        ...
        this.bullets = [];
        this.enemies = [];
        this.toxicTrails = []; // NEW: Reset trails
        this.keys = {};
        this.score = 0;
        this.gameOver = false;
        this.isPaused = false;
    ...
    public update(deltaTime: number) { // MODIFIED: Accepts deltaTime from component
        ...
        // NEW: TOXIC TRAILS UPDATE LOGIC
        this.updateToxicTrails(Date.now());
    ...
    /**
     * Handles the creation, growth, and cleanup of toxic puddles.
     */
    private updateToxicTrails(currentTime: number) {
        // Spawn segments from bullets marked 'hasTrail'
        this.bullets.forEach(bullet => {
            if (bullet.hasTrail) {
                this.toxicTrails.push({
                    x: bullet.x,
                    y: bullet.y,
                    radius: 5,
                    maxRadius: 20,
        ...
        // Update lifecycle and collision
        for (let i = this.toxicTrails.length - 1; i >= 0; i--) {
            const t = this.toxicTrails[i];
            const age = currentTime - t.creationTime;
            const lifeRatio = age / t.lifespan;

            // Toxic Effect: Puddle grows while fading out
            t.radius = 5 + (t.maxRadius - 5) * lifeRatio;
            t.opacity = 0.6 * (1 - lifeRatio);

            // Cleanup dead segments
            if (age > t.lifespan) {
                this.toxicTrails.splice(i, 1);
                continue;
            }

            // Damage Player if they step in the goo
            const dx = this.player.x - t.x;
            const dy = this.player.y - t.y;
            const dist = Math.sqrt(dx * dx + dy * dy);
            if (dist < t.radius + this.player.radius) {
                this.player.health -= 0.15; // Rapid tick damage
            }
        }
    }
